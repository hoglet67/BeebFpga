...
static int ulpi_wait(struct ulpi_viewport *ulpi_vp, u32 mask)
{
	...
	while (--timeout) {
		if ((Xil_In32(ulpi_vp->viewport_addr) & mask) == 0)
			return 0;
...
static int ulpi_wakeup(struct ulpi_viewport *ulpi_vp)
{
	...
	if (Xil_In32(ulpi_vp->viewport_addr) & ULPI_SS)
		return 0; /* already awake */

	Xil_Out32(ulpi_vp->viewport_addr, ULPI_WU);
...
static int ulpi_request(struct ulpi_viewport *ulpi_vp, u32 value)
{
	...
	Xil_Out32(ulpi_vp->viewport_addr, value);
...
u32 ulpi_read(struct ulpi_viewport *ulpi_vp, u8 *reg)
{
	...
	return (Xil_In32(ulpi_vp->viewport_addr) >> 8) & 0xff;
}
